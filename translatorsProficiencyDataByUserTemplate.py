...
def find_babel_languages(title):
    ...
    if response.status_code == 200:
        ...
        user_templates = re.findall(r"\{\{User(?![^:}]*?:)(?:[^}]*?)[^\w}](.*?)\}\}", content, re.IGNORECASE)

        language_claims = []
        if user_templates:
            valid_user_languages = []  # Initialize an empty list
            for lang in user_templates:
                if '-' in lang:
                    language_code, proficiency = lang.rsplit('-', 1)
                    if language_code in allowed_languages:
                        valid_user_languages.append(lang)  # Append to the list
                else:
...
output_file = 'user_languages_user_template.csv.csv'
